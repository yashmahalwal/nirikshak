...
describe("Path joiner class", () => {
    ...
    test("Effeciently caches the values", async () => {
        const j = new PathJoiner();
        const p1 = samplePaths[Math.floor(Math.random() * samplePaths.length)];
        const p2 = samplePaths[Math.floor(Math.random() * samplePaths.length)];
