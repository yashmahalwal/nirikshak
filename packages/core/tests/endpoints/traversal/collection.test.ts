...
} from "../../../src/endpoints/traversal/collection";
...
describe(`Collection traversal`, () => {
  ...
  for (let i = 1; i <= RANDOMNESS_ITERATIONS; i++) {
    ...
    test(`Get random non existing resource: ${i}`, async () => {
      const got = await getRandomNewInstance(ValidResource, Helpers);
      expect(collection.has(got.id)).toBe(false);
    });
  }
