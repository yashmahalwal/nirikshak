...
export async function matchBase(
  ...
  for (let i = 0; i < base.length; i++)
    if (!(await matchBase(input[i], base[i], resource, helpers))) return false;
  return true;
}
...
export async function matchOneOfEntries(
  ...
  if (input === undefined) {
    ...
    if ("fields" in schema)
      for (const field of schema.fields)
        if (isWithModifiersBodyType(field) && field.optional) return true;
    return false;
  }
  ...
  if (input === null) {
    if ("types" in schema)
      for (const type of schema.types)
        if ((isWithModifiersBase(type) && type.nullable) || type === null)
          return true;
    if ("fields" in schema)
      for (const field of schema.fields)
        if (isWithModifiersBodyType(field) && field.nullable) return true;
    return false;
  }
  ...
  if (isPrimitives(input)) {
    ...
    if ("types" in schema)
      for (const type of schema.types)
        if (isBase(type)) {
          if (await matchBase(input, type, resource, helpers)) return true;
        } else if (await matchWithModifiersBase(input, type, resource, helpers))
          return true;
  } else if ("fields" in schema) {
    for (const field of schema.fields) {
      if (isWithModifiersBodyType(field)) {
        if (await matchWithModifiersBodyType(input, field, resource, helpers))
          return true;
      } else if (await matchBody(input, field, resource, helpers)) return true;
    }
  }
    ...
    if (isResourceString(entry)) {
      ...
      value =
        entry.optional && body[key] === undefined
          ? true
          : await matchWithModifiersBase(body[key], entry, resource, helpers);
      ...
      value = await matchOneOfEntries(body[key], entry, resource, helpers);
